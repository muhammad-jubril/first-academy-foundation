(function () {
  ...
  function onScrollHeader() {
    if (!header) return;
    if (window.scrollY > 10) header.style.boxShadow = "0 4px 20px rgba(27,59,34,.12)";
    else header.style.boxShadow = "";
  }
  document.addEventListener("scroll", onScrollHeader, { passive: true });

  /* ---------- In-page anchor highlighting (for pages with #id sections,
     e.g. academics.html#nursery). Cross-page active state is already
     rendered server-side via the is-active class on each page. ---------- */
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      /* No backend is connected yet — this confirms the form works client-side.
         Wire this up to an email service (e.g. Formspree, EmailJS) or your own
